...
# ═══════════════════════════════════════════════════════════════════════════════
# Advanced Dynamic Etch — Raspberry Pi Controller
# For use with: Blue Pill (STM32F103C8) + DM542 stepper driver
#
# Changes from Arduino/A4988 version:
#   - Serial port: /dev/ttyAMA0 (UART) instead of /dev/ttyACM0 (USB)
#   - Ensure Pi serial console is disabled: raspi-config → Interface Options
#     → Serial → "No" for login shell, "Yes" for serial hardware.
# ═══════════════════════════════════════════════════════════════════════════════
...
# --- SERIAL PORT ---
# Blue Pill communicates via USART1 (PA9/PA10) over UART, not USB.
# Pi side: /dev/ttyAMA0  (or /dev/serial0 on some Pi models)
...
try:
    print(f"Connecting to Blue Pill on {SERIAL_PORT} (UART)...")
    controller = serial.Serial(SERIAL_PORT, BAUD_RATE, timeout=1)
    time.sleep(2)
    print("Connected to Blue Pill!")
except Exception as e:
    print("Failed to connect to Blue Pill:", e)
    controller = None
        ...
        if avg_c < CC and len(history) == HISTORY_LEN:
            ...
            if elapsed_in_state >= STATIC1_DURATION:
                ...
                if controller:
                    ...
                    controller.write(cmd.encode('utf-8'))
            motor_speed = 0.0
            ...
            relay_state = 1
            if controller:
                controller.write(b"V 0\n") # Stop motor
                # Optionally move up slightly to clear the liquid
                # controller.write(b"U 5000 100\n")
            motor_speed = 0.0
